...
for(i in 1:length(years)){
  ...
  text_val[[paste0("text_val_", years[i])]] <- c(round(seq(0, hvds[[i]], length.out = sum(colour[[i]] != "no"))),
                                                 rep(hvds[[i]], sum(colour[[i]] == "no")))
}
...
p + 
  ...
  geom_text(data = static_text_data,
            aes(x = x_value, y = y, label = static_text),
...
## Export final plot ----
ggsave(filename = file.path("../plots", "202102_webdubois.png"), 
       dpi = 600, width = 6.15, height = 7.7)

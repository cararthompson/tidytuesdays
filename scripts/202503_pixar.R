...
tuesdata$public_response |>
  ...
  dplyr::mutate(film_colour = dplyr::case_when(grepl("Toy Story", film) ~ "#feca36",
  ...
  geom_segment(aes(xend = release_date,
                   y = rotten_tomatoes,
                   alpha = release_date,
                   yend = critics_choice),
               colour = "#bbced2") + 
  ...
  ggfx::with_inner_glow(
    geom_point(aes(y = rotten_tomatoes,
                   size = run_time,
                   fill = film_colour,
                   colour = film_colour,
                   alpha = release_date),
               shape = 21),
    "#e8f5fb", expand = 3, sigma = 20) +
  ...
  ggfx::with_outer_glow(
    ggtext::geom_textbox(data = data.frame(),
                         aes(x = lubridate::ymd(c("1995-01-01", 
                                                  "2005-01-01", 
                                                  "2015-01-01")),
                             y = rep(25, 3),
  ...
  ggfx::with_outer_glow(
    ggtext::geom_textbox(data = data.frame(),
                         aes(x = lubridate::ymd("1990-01-01"),
                             y = c(50, 75, 100, 102.5),
  ...
  ggfx::with_outer_glow(
    ggtext::geom_textbox(data = data.frame(),
                         aes(x = lubridate::ymd("1992-01-01"),
                             y = 50,
                         ...
                         fill = NA,
                         box.colour = NA),
    colour = "#56547c", sigma = 10) +
  ggfx::with_outer_glow(
    ggtext::geom_textbox(data = data.frame(),
                         aes(x = lubridate::ymd("2012-01-01"),
                             y = 55,
                         ...
                         fill = NA,
                         box.colour = NA),
    colour = "#56547c", sigma = 12) +
  ggfx::with_outer_glow(
    ggtext::geom_textbox(data = data.frame(),
                         aes(x = lubridate::ymd("1995-11-22"),
                             y = 95,
    ...
    colour = "#56547c", sigma = 12) +
  ggfx::with_outer_glow(
    geom_segment(x = lubridate::ymd("1995-11-22"), xend = lubridate::ymd("1995-11-22"),
                 y = 98, yend = 99,
                 colour = "#bbced2",
                 linewidth = 0.5),
    colour = "#56547c", sigma = 10) +
  labs(caption = "#TidyTuesday | Dataviz: Cara Thompson, Building Stories with Data LTD | Source: {pixarfilms}") +
  theme_void() +
  theme(plot.background = element_rect(fill = grid::linearGradient(c("#c8cce7", "#558fd1")),
                                       colour = "#efeae6", linewidth = 10),
        plot.margin = margin(rep(5, 4)),
        plot.caption = element_text(colour = "#56547c", margin = margin(0, 0, 20, 0), hjust = 0.5, family = "Commissioner", face = "bold"),
...
# Final export
ggsave(filename = here::here("plots", "202503_pixar.png"), 
       dpi = 400, width = 6.8, height = 10, bg = "#ffffff")
